...
def load_data():
    ...
    if opt.images_mode == 'images_test':
        ...
        train_ds = tf.data.Dataset.list_files('facescrub/images_test/*.jpeg', shuffle=False)  # images images_test
    else:
        # train_ds = tf.data.Dataset.list_files('miniimagenet/images/*.jpg', shuffle=False)  # images images_test
        train_ds = tf.data.Dataset.list_files('facescrub/images/*.jpeg', shuffle=False)  # images images_test
    # train_ds = tf.data.Dataset.list_files('miniimagenet/images_test/*.jpg', shuffle=False) #images images_test
...
def optimize_pattern_with_data(opt):
    ...
    for fileindex in train_ds_name:
        if opt.images_mode == 'images_test':
            file_name.append(str(fileindex.numpy())[24:-1].encode('ascii').decode('unicode_escape').encode('latin-1').decode('utf-8'))  # python的切片左闭右开
        else:
            file_name.append(str(fileindex.numpy())[19:-1].encode('ascii').decode('unicode_escape').encode('latin-1').decode('utf-8'))
    ...
    if opt.pretrained == 'inception_resnetv1_dec':
        model_gradcam = InceptionResnetV1(classify=True, pretrained='facescrub_dec', num_classes=530, device=device).to(device)
        target_layers = [model_gradcam.repeat_3]
    elif opt.pretrained == 'inception_resnetv1_cap':
        model_gradcam = InceptionResnetV1(classify=True, pretrained='facescrub_cap', num_classes=530, device=device).to(device)
        target_layers = [model_gradcam.repeat_3]
    ...
    if opt.pretrained == 'inception_resnetv1_dec':
        model = InceptionResnetV1(classify=True, pretrained='facescrub_dec', num_classes=530, device=device).to(device)
    elif opt.pretrained == 'inception_resnetv1_cap':
        model = InceptionResnetV1(classify=True, pretrained='facescrub_cap', num_classes=530, device=device).to(device)
        # fc_features = model.fc.in_features
        # model.fc = nn.Linear(fc_features, 100)
        ...
        for batch_id, batch_img in enumerate(train_ds):
            ...
            while (location_flag):
                with tf.GradientTape() as g:
                    ...
                    # One pixel denfender
                    # interfer_pixel[84] = 3  ##green
                    # interfer_pixel[85] = 3 ##blue
                    ...
                    ############# mtcnn batch_img
                    # batch_img_test = vis.tensor_to_img_save(batch_img, 0)
                    # batch_img_test = Image.fromarray(batch_img_test)
                    # face_boxes, face_probs = mtcnn.detect(batch_img_test)
                    # # Draw boxes and save faces
                    # img_draw = batch_img_test.copy()
                    # draw = ImageDraw.Draw(img_draw)
                    # for i, box in enumerate(face_boxes):
                    #     draw.rectangle(box.tolist(), width=5)
                    # extract_face(batch_img_test, box, save_path='detected_face_{}.png'.format(i))
                    # # for i, (box, point) in enumerate(zip(face_boxes, face_points)):
                    # #     draw.rectangle(box.tolist(), width=5)
                    # #     for p in point:
                    ...
                    if face_detect_flag == 1:
                        ...
                        if face_boxes is None: # face_boxes.size == 0:
                            ...
                            batch_img_temp = random_size(batch_img_temp, target_size=160)
                            batch_img_temp = center_crop(batch_img_temp)
                            ...
                            # face_boxes = [
                            #     int(max(face_boxes[0][0] - 0 / 2, 0)),
                            #     int(max(face_boxes[0][1] - 0 / 2, 0)),
                            #     int(min(face_boxes[0][2] + 0 / 2, raw_image_size[0])),
                            #     int(min(face_boxes[0][3] + 0 / 2, raw_image_size[1])),
                            # ]
                            # draw.rectangle(face_boxes,width=5) # The parameter specifies the diagonal vertices (upper left and lower right) of the rectangle, in the form (x0, y0, x1, y1)
                            # # for i, box in enumerate(face_boxes):
                    ...
                    if location_flag == 1:
                        ...
                        #
                        # RGB_face_detect_mask = np.stack([face_detect_mask, face_detect_mask, face_detect_mask],
                        #                           axis=2)
                        #
                        # plt.imshow(face_detect_mask)
                        # plt.show()
                        ...
                        if face_boxes is not None:  # if len(face_boxes) != 0:
                            ...
                            batch_img_temp = face_crop(batch_img, face_boxes)
                            ...
                            img_tf_temp = face_crop(img_tf, face_boxes)
                        else:
                            img_tf = tf.io.read_file(img_path)
                            img_tf = tf.image.decode_jpeg(img_tf, channels=3, dct_method='INTEGER_ACCURATE')
                            img_tf = tf.cast(img_tf, dtype=tf.float32) / 255
                        ...
                        interfer_img_index = center_crop_numpy(interfer_img_index)
                        img_tf_temp = random_size(img_tf_temp, target_size=160)
                        img_tf_temp = center_crop(img_tf_temp)
                        batch_img_temp = random_size(batch_img_temp, target_size=160)
                        batch_img_temp = center_crop(batch_img_temp)
                        captured = random_size(captured, target_size=160)
                        captured = center_crop(captured)
                        ...
                        deconved = center_crop(deconved)
                        for iiii in range(batch_size):
                            img_saveface = vis.tensor_to_img_save(img_tf_temp, iiii)  # captured image in current batch

                        # img_path = "/data/volume_2/optimize_display_POLED_400PPI/logs/test/web/images/process_temp/temp.jpg" # opt.save_temp_dir
                        img_path = os.path.join(img_temp_dir, 'temp.jpg')
                        # Util.save_image(img_saveface, img_path)
                        image_pil = None
                        if img_saveface.shape[2] == 1:
                            img_saveface = np.reshape(img_saveface, (img_saveface.shape[0], img_saveface.shape[1]))
                            image_pil = Image.fromarray(img_saveface, 'L')
                        else:
                            image_pil = Image.fromarray(img_saveface)
                        image_pil.save(img_path)
                        img = Image.open(img_path).convert('RGB')
                        ...
                        # img = random_size_numpy(img, target_size=160)
                        # img = center_crop_numpy(img)
                        ...
                        ############# mtcnn deconved_img
                        # img = Image.fromarray(deconved_img)
                        # plt.imshow(img)
                        # plt.show()
                        #
                        # face_boxes, face_probs = mtcnn.detect(img)
                        ...
                        loss, _ = criterion(mapped_pattern, PSFs, PSFs_RGB, ssim_mean, deconved, batch_img_temp,
                                            epoch=epoch_id)
            ...
            for iiii in range(batch_size):
                ...
                if opt.save_mode == 'all':
                    visuals['captured_0_all'] = captured_img  # Save original size image
                    visuals['deconved_0_all'] = deconved_img  # Save original size image
                elif opt.save_mode == 'crop':
                    visuals['captured_0_crop'] = vis.tensor_to_img_save(captured, iiii)  # Save cropped size image
                    visuals['deconved_0_crop'] = vis.tensor_to_img_save(deconved, iiii)  # Save cropped size image
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_cap_dir', type=str, default='cap_onepixel_face_test',
                        help='save perturbed images')  # cap_allcolorbar_p10_v1
    parser.add_argument('--save_dec_dir', type=str, default='dec_onepixel_face_test',
                        help='save deblurred images')  # dec_allcolorbar_p10_v1
    parser.add_argument('--save_temp_dir', type=str, default='onepixel_face_test_process_temp',
    ...
    parser.add_argument('--pretrained', type=str, default='inception_resnetv1', help='pretrained network model')
