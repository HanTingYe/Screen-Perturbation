...
def optimize_pattern_with_data(opt):
    ...
    for ii in range(len(all_interfer_pixel_index)):
        if all_interfer_pixel_index[ii] % 3 == 2:
            ...
            all_interfer_pixel_red_index[ii] = 0.66  # power for red pixel
        elif all_interfer_pixel_index[ii] % 3 == 0:
            all_interfer_pixel_trans_index[ii] = 3  # Green
            all_interfer_pixel_green_index[ii] = 0.66  # power for red pixel
        elif all_interfer_pixel_index[ii] % 3 == 1:
            all_interfer_pixel_trans_index[ii] = 4  # Blue
            all_interfer_pixel_blue_index[ii] = 0.66  # power for red pixel
    ...
    assert os.path.exists(weights_path_gradcam), "file: '{}' dose not exist.".format(weights_path)
        ...
        for batch_id, batch_img in enumerate(train_ds):
            ...
            while (location_flag):
                with tf.GradientTape() as g:
                    ...
                    img_path = "/data/volume_2/optimize_display_POLED_400PPI/logs/test/web/images/process_temp/temp.jpg"
                    ...
                    if (color_flag == 1):
                        ...
                        if (target_category == target_category_ori): #target_category == target_category_ori judge_test[iter_count-1] < 100
                            if (green_flag == 1):
                                    ...
                                    if (blue_flag == 2):
                                        if (intensity_flag == 1):
                                            ...
                                            if interfer_pixel[interfer_pixel_index == greenopt_pixel_mask_index[
                                                np.argmin(greenpixels_prob)]] > max_brightness and interfer_pixel[
                                                interfer_pixel_index == blueopt_pixel_mask_index[
                                                    np.argmin(np.absolute(
                                                        blueopt_pixel_mask_index - greenopt_pixel_mask_index[
                                                            np.argmin(greenpixels_prob)]))]] > max_brightness:
                                                location_flag = 0
                                    elif (red_flag == 2):
                                        if (intensity_flag == 1):
                                            interfer_pixel = np.zeros(np.shape(interfer_pixel_index), dtype=np.float32)
                                    ...
                                    if (red_flag == 2):
                                        if (intensity_flag == 1):
                                            ...
                                            if interfer_pixel[interfer_pixel_index == blueopt_pixel_mask_index[
                                                np.argmin(bluepixels_prob)]] > max_brightness and interfer_pixel[
                                                interfer_pixel_index == redopt_pixel_mask_index[
                                                    np.argmin(np.absolute(
                                                        redopt_pixel_mask_index - blueopt_pixel_mask_index[
                                                            np.argmin(bluepixels_prob)]))]] > max_brightness:
                                                location_flag = 0
                                    else:
                                        if (intensity_flag == 1):
                                            interfer_pixel = np.zeros(np.shape(interfer_pixel_index), dtype=np.float32)
                                ...
                                if (redpixels_count < redpixels_len):
                                    interfer_pixel = np.zeros(np.shape(interfer_pixel_index), dtype=np.float32)
                                    interfer_pixel[
                                        interfer_pixel_index == redopt_pixel_mask_index[redpixels_count]] = ini_intensity
                                    redpixels_prob[redpixels_count] = judge_crteria
                                    redpixels_count += 1
                                else:
                                    if (intensity_flag == 1):
                                        interfer_pixel = np.zeros(np.shape(interfer_pixel_index), dtype=np.float32)
                                        ...
                                        if interfer_pixel[interfer_pixel_index == redopt_pixel_mask_index[
                                            np.argmin(redpixels_prob)]] > max_brightness:
                                            location_flag = 0
