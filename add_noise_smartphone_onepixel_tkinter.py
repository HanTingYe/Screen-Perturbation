...
def optimize_pattern_with_data(opt):
    ...
    txt_path = os.path.join(img_temp_dir, 'orders.txt')
        ...
        for batch_id, batch_img in enumerate(train_ds):
            ...
            while (location_flag):
                with tf.GradientTape() as g:
                    ...
                    # ## smartphone
                    if (color_flag == 1):
                        interfer_pixel_index = np.arange(15984).reshape(5328, 3).flatten()  # np.zeros((1, 30), dtype=int)
                        # interfer_pixel = np.zeros(np.shape(interfer_pixel_index), dtype=np.float32)
                        interfer_pixel = 255*np.ones(np.shape(interfer_pixel_index), dtype=np.float32)
                    ...
                    t = threading.Thread(target=display_image, args=())  # 参数2为元组,是传递给run()方法的参数值,无参数传递可去掉第2个参数
                    ...
                    # ImageStreamingTest(hort, port, cap_path)
                    # **********end
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_cap_dir', type=str, default='cap_onepixel_test',
                        help='save perturbed images')  # cap_allcolorbar_p10_v1
    parser.add_argument('--save_dec_dir', type=str, default='dec_onepixel_test',
                        help='save deblurred images')  # dec_allcolorbar_p10_v1
    parser.add_argument('--save_temp_dir', type=str, default='onepixel_test_process_temp',
