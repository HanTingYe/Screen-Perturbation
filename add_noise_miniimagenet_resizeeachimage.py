...
def random_size(image, target_size=None):
    ...
    # resize_shape = (int(height.numpy() * size_ratio.numpy()), int(width.numpy() * size_ratio.numpy())) # width and height in cv2 are opposite to np.shape()
...
def load_data():
    ...
    train_ds = tf.data.Dataset.list_files('miniimagenet/images/*.jpg', shuffle=False)
        ...
        for batch_id, batch_img in enumerate(train_ds):
            with tf.GradientTape() as g:
                ...
                display_interference_pre = crop_image(
                    ...
                    img_ori_height, img_ori_width)  # INTER_LINEAR INTER_AREA
                ...
                for ii in range(len(interfer_pixel_index)):
                    if interfer_pixel_index[ii] % 3 == 2:
                        ...
                        interfer_pixel_red_index[ii] = 2 # power for red pixel
                    elif interfer_pixel_index[ii] % 3 == 0:
                        interfer_pixel_trans_index[ii] = 3 # Green
                        interfer_pixel_green_index[ii] = 2  # power for red pixel
                    elif interfer_pixel_index[ii] % 3 == 1:
                        interfer_pixel_trans_index[ii] = 4 # Blue
                        interfer_pixel_blue_index[ii] = 2  # power for red pixel
                ...
                # for ii in range(len(interfer_pixel_index)):
                #     all_interfer_pixel = np.where(all_interfer_pixel_index == interfer_pixel_index[ii], interfer_pixel[ii], all_interfer_pixel)
                ...
                for iii in range(len(all_interfer_pixel_index)):
                    mapped_u1_RGB_real_mask = tf.where(
                        u1_RGB_real_mask == tf.constant(all_interfer_pixel_index[iii], dtype=tf.float32), mapped_all_pixel[iii], mapped_u1_RGB_real_mask)
                ...
                batch_img = random_size(batch_img, target_size=256)
                batch_img = center_crop(batch_img)
                ...
                loss, _ = criterion(mapped_pattern, PSFs, PSFs_RGB, deconved, batch_img, epoch=epoch_id)
