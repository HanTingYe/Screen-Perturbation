...
device_gradcam = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
...
def random_size_numpy(image, target_size=None):
    ...
    if height < width:
        ...
        size_ratio = target_size / width
    resize_shape = (int(width * size_ratio), int(height * size_ratio)) ## Right one
...
def set_interfer_pixel(interfer_pixel, colorstatusbar_flag, interfer_pixel_red_index, interfer_pixel_green_index, interfer_pixel_blue_index) :
...
def optimize_pattern_with_data(opt):
    ...
    for fileindex in train_ds_name:
        file_name.append(str(fileindex.numpy())[22:43]) #python的切片左闭右开
        ...
        for batch_id, batch_img in enumerate(train_ds):
            ...
            while (location_flag):
                with tf.GradientTape() as g:
                    ...
                    if (color_flag == 1):
                        interfer_pixel = np.zeros(np.shape(interfer_pixel_index), dtype=np.float32)
                        interfer_pixel = set_interfer_pixel(interfer_pixel, colorstatusbar_flag, interfer_pixel_red_index,
                    ...
                    for iii in range(len(all_red_pixel_index[0])):
                        all_display_red_mask_ssim = tf.where(all_display_red_mask_ssim == all_interfer_pixel_index[all_red_pixel_index[0][iii]],
                    ...
                    all_display_RGB_mask_ssim = np.stack([all_display_red_mask_ssim, all_display_green_mask_ssim, all_display_blue_mask_ssim],
                                            axis=2)
                    ...
                    Z_sast = math.pow(math.sqrt(1 / (4*math.tan(theta_h/2*math.pi/180)*math.tan(theta_w/2*math.pi/180))*math.pow(1/Obs_d,2)/gamma_DOR), (1-math.pow(math.fabs(gamma_DOR-gamma_o),beta_z)/alpha_z)) # (1-math.pow(math.fabs(gamma_DOR-gamma_o),beta_z)/alpha_z)
                    # Z_sast = 1
                    ...
                    all_display_RGB_mask_ssim_new = cv2.resize(all_display_RGB_mask_ssim_blur, None, None, fx=Z_sast, fy=Z_sast, interpolation=cv2.INTER_LINEAR)#INTER_LINEAR INTER_CUBIC
                    ...
                    ssim_value, cs_map_value =tf_ssim(all_display_RGB_mask_ori_old, all_display_RGB_mask_ssim_new, cs_map=True, mean_metric=False, filter_size=11,
                            filter_sigma=1.5)
                    ...
                    # plt.show()
                    PSFs, PSFs_RGB, interfer_all_pixel, u1_RGB_real = camera(mapped_u1_RGB_real_mask,
                                                                             mode_option='running')
                    ...
                    if (color_flag == 1):
                        ...
                        grayscale_cam = cam(input_tensor=input_tensor, target_category=target_category_ori)#target_category_ori
                        ...
                        redopt_respixel_mask_index = np.setdiff1d(redopt_allpixel_mask_index, redopt_pixel_mask_index)
                        ...
                        greenopt_allpixel_mask_index = greenopt_allpixel_mask_index[greenopt_allpixel_mask_index != 0]
                        ...
                        greenopt_respixel_mask_index = np.setdiff1d(greenopt_allpixel_mask_index, greenopt_pixel_mask_index)
                        ...
                        blueopt_respixel_mask_index = np.setdiff1d(blueopt_allpixel_mask_index, blueopt_pixel_mask_index)
                            ...
                            if (green_flag == 1):
                                if (greenpixels_count <= greenpixels_len):
                                    ...
                                    if (blue_flag == 2):
                                        if (intensity_flag == 1):
                                            ...
                                            pickinterfer_greenopt_index[greenopt_pickindex] = np.where(interfer_pixel_index == greenopt_pixel_mask_index[
                                                    np.argmin(greenpixels_prob)])[0]
                                            # picked optimal blue location
                                            pickinterfer_blueopt_index[blueopt_pickindex] = np.where(interfer_pixel_index == blueopt_pixel_mask_index[
                                                np.argmin(np.absolute(
                                                    blueopt_pixel_mask_index - greenopt_pixel_mask_index[
                                                        np.argmin(greenpixels_prob)]))])[0]


                                            interfer_pixel[pickinterfer_greenopt_index[0:greenopt_pickindex+1]] = ini_intensity ## python slice left closed right open python的切片左闭右开
                                            interfer_pixel[pickinterfer_blueopt_index[0:blueopt_pickindex+1]] = ini_intensity

                                            ## background status bar color change
                                            if any(redopt_respixel_mask_index):
                                                # picked optimal red location
                                                pickinterfer_redopt_index[redopt_pickindex] = \
                                                    np.where(interfer_pixel_index == redopt_respixel_mask_index[
                                                        np.argmax(np.absolute(
                                                            redopt_respixel_mask_index - greenopt_pixel_mask_index[
                                                                np.argmin(greenpixels_prob)]))])[0]
                                                interfer_pixel[pickinterfer_redopt_index[
                                                               0:redopt_pickindex + 1]] -= ini_intensity * 2
                                            ...
                                            interfer_pixel[pickinterfer_greenopt_index[0:greenopt_pickindex+1]] += step_size
                                            # interfer_pixel[interfer_pixel_index == blueopt_pixel_mask_index[
                                            #     np.argmin(bluepixels_prob)]] += step_size
                                            interfer_pixel[pickinterfer_blueopt_index[0:blueopt_pickindex+1]] += step_size

                                            ## background status bar color change (max_brightness/2 + step_size) 0
                                            if any(redopt_respixel_mask_index):
                                                interfer_pixel[
                                                    pickinterfer_redopt_index[0:redopt_pickindex + 1]] -= step_size * 2
                                                if np.all(interfer_pixel[
                                                              pickinterfer_redopt_index[0:redopt_pickindex + 1]] < 0):
                                                    interfer_pixel[
                                                        pickinterfer_redopt_index[0:redopt_pickindex + 1]] = 0
                                            ...
                                            if np.all(interfer_pixel[pickinterfer_greenopt_index[0:greenopt_pickindex+1]] > (max_brightness + step_size)) and np.all(interfer_pixel[
                                                pickinterfer_blueopt_index[0:blueopt_pickindex+1]] > (max_brightness + step_size)):

                                                if greenopt_pickindex < greenpixels_len - 1 or blueopt_pickindex < bluepixels_len - 1:
                                                    intensity_flag = 1
                                                else:
                                                    location_flag = 0
                                                ...
                                                if blueopt_pickindex < bluepixels_len - 1:
                                                    blueopt_pickindex += 1
                                                    blueopt_pixel_mask_index = np.delete(blueopt_pixel_mask_index, np.argmin(np.absolute(
                                                    blueopt_pixel_mask_index - greenopt_pixel_mask_index[
                                                        np.argmin(greenpixels_prob)])))

                                                if redopt_pickindex < redpixels_len - 1:
                                                    redopt_pickindex += 1
                                                    redopt_pixel_mask_index = np.delete(redopt_pixel_mask_index, np.argmin(np.absolute(
                                                    redopt_pixel_mask_index - greenopt_pixel_mask_index[
                                                        np.argmin(greenpixels_prob)])))

                                                if greenopt_pickindex < greenpixels_len - 1:
                                                    greenopt_pickindex += 1
                                                    greenopt_pixel_mask_index = np.delete(greenopt_pixel_mask_index,np.argmin(greenpixels_prob))
                                                    greenpixels_prob = np.delete(greenpixels_prob,np.argmin(greenpixels_prob))
                                        ...
                                        if (intensity_flag == 1):
                                            ...
                                            interfer_pixel[pickinterfer_greenopt_index[
                                                           0:greenopt_pickindex + 1]] = ini_intensity  ## python slice left closed right open python的切片左闭右开
                                            interfer_pixel[
                                                pickinterfer_redopt_index[0:redopt_pickindex + 1]] = ini_intensity

                                            ## background status bar color change
                                            if any(blueopt_respixel_mask_index):
                                                # picked optimal blue location
                                                pickinterfer_blueopt_index[blueopt_pickindex] = \
                                                    np.where(interfer_pixel_index == blueopt_respixel_mask_index[
                                                        np.argmax(np.absolute(
                                                            blueopt_respixel_mask_index - greenopt_pixel_mask_index[
                                                                np.argmin(greenpixels_prob)]))])[0]
                                                interfer_pixel[
                                                    pickinterfer_blueopt_index[
                                                    0:blueopt_pickindex + 1]] -= ini_intensity * 2
                                            ...
                                            interfer_pixel = interfer_pixel.numpy()
                                            interfer_pixel[
                                                pickinterfer_greenopt_index[0:greenopt_pickindex + 1]] += step_size
                                            interfer_pixel[
                                                pickinterfer_redopt_index[0:redopt_pickindex + 1]] += step_size

                                            ## background status bar color change (max_brightness/2 + step_size) 0
                                            if any(blueopt_respixel_mask_index):
                                                interfer_pixel[
                                                    pickinterfer_blueopt_index[
                                                    0:blueopt_pickindex + 1]] -= step_size * 2
                                                if np.all(interfer_pixel[
                                                              pickinterfer_blueopt_index[0:blueopt_pickindex + 1]] < 0):
                                                    interfer_pixel[
                                                        pickinterfer_blueopt_index[0:blueopt_pickindex + 1]] = 0
                                            ...
                                            #             np.argmin(greenpixels_prob)]))]]
                                            if np.all(interfer_pixel[
                                                          pickinterfer_greenopt_index[0:greenopt_pickindex + 1]] > (
                                                              max_brightness + step_size)) and np.all(interfer_pixel[
                                                                                                          pickinterfer_redopt_index[
                                                                                                          0:redopt_pickindex + 1]] > (
                                                                                                              max_brightness + step_size)):

                                                if greenopt_pickindex < greenpixels_len - 1 or redopt_pickindex < redpixels_len - 1:
                                                    intensity_flag = 1
                                                else:
                                                    location_flag = 0
                                        ...
                                        if (intensity_flag == 1):
                                            ...
                                            if any(redopt_respixel_mask_index):
                                                ...
                                                interfer_pixel[
                                                    pickinterfer_redopt_index[
                                                    0:redopt_pickindex + 1]] -= ini_intensity
                                ...
                                if (bluepixels_count <= bluepixels_len):
                                    ...
                                    if (red_flag == 2):
                                        if (intensity_flag == 1):
                                            ...
                                            interfer_pixel[pickinterfer_blueopt_index[
                                                           0:blueopt_pickindex + 1]] = ini_intensity  ## python slice left closed right open python的切片左闭右开
                                            interfer_pixel[
                                                pickinterfer_redopt_index[0:redopt_pickindex + 1]] = ini_intensity

                                            ## background status bar color change
                                            if any(greenopt_respixel_mask_index):
                                                # picked optimal green location
                                                pickinterfer_greenopt_index[greenopt_pickindex] = \
                                                    np.where(interfer_pixel_index == greenopt_respixel_mask_index[
                                                        np.argmax(np.absolute(
                                                            greenopt_respixel_mask_index - blueopt_pixel_mask_index[
                                                                np.argmin(bluepixels_prob)]))])[0]
                                                interfer_pixel[
                                                    pickinterfer_greenopt_index[
                                                    0:greenopt_pickindex + 1]] -= ini_intensity * 2
                                            ...
                                            interfer_pixel = interfer_pixel.numpy()
                                            interfer_pixel[
                                                pickinterfer_blueopt_index[0:blueopt_pickindex + 1]] += step_size
                                            interfer_pixel[
                                                pickinterfer_redopt_index[0:redopt_pickindex + 1]] += step_size
                                            ...
                                            if any(greenopt_respixel_mask_index):
                                                interfer_pixel[
                                                    pickinterfer_greenopt_index[
                                                    0:greenopt_pickindex + 1]] -= step_size * 2
                                                if np.all(interfer_pixel[
                                                              pickinterfer_greenopt_index[
                                                              0:greenopt_pickindex + 1]] < 0):
                                                    interfer_pixel[
                                                        pickinterfer_greenopt_index[0:greenopt_pickindex + 1]] = 0
                                            ...
                                            #             np.argmin(greenpixels_prob)]))]]
                                            if np.all(interfer_pixel[
                                                          pickinterfer_blueopt_index[0:blueopt_pickindex + 1]] > (
                                                              max_brightness + step_size)) and np.all(interfer_pixel[
                                                                                                          pickinterfer_redopt_index[
                                                                                                          0:redopt_pickindex + 1]] > (
                                                                                                              max_brightness + step_size)):

                                                if blueopt_pickindex < bluepixels_len - 1 or redopt_pickindex < redpixels_len - 1:
                                                    intensity_flag = 1
                                                else:
                                                    location_flag = 0
                                        ...
                                        if (intensity_flag == 1):
                                            ...
                                            if any(greenopt_respixel_mask_index):
                                                ...
                                                interfer_pixel[
                                                    pickinterfer_greenopt_index[
                                                    0:greenopt_pickindex + 1]] -= ini_intensity
                                            ...
                                            if any(greenopt_respixel_mask_index):
                                                interfer_pixel[
                                                    pickinterfer_greenopt_index[
                                                    0:greenopt_pickindex + 1]] -= step_size
                                                if np.all(interfer_pixel[
                                                              pickinterfer_greenopt_index[
                                                              0:greenopt_pickindex + 1]] < 0):
                                                    interfer_pixel[
                                ...
                                if (redpixels_count <= redpixels_len):
                                    ...
                                    if (intensity_flag == 1):
                                        ...
                                        interfer_pixel[pickinterfer_redopt_index[
                                                       0:redopt_pickindex + 1]] = ini_intensity  ## python slice left closed right open python的切片左闭右开
                                        ...
                                        if any(blueopt_respixel_mask_index):
                                            ...
                                            pickinterfer_blueopt_index[blueopt_pickindex] = \
                                                np.where(interfer_pixel_index == blueopt_respixel_mask_index[
                                                    np.argmax(np.absolute(
                                                        blueopt_respixel_mask_index - redopt_pixel_mask_index[
                                                            np.argmin(redpixels_prob)]))])[0]
                                            interfer_pixel[
                                                pickinterfer_blueopt_index[
                                                0:blueopt_pickindex + 1]] -= ini_intensity
                                        ...
                                        if np.all(interfer_pixel[pickinterfer_redopt_index[0:redopt_pickindex + 1]] > (max_brightness + step_size)):
                    ...
                    loss, _ = criterion(mapped_pattern, PSFs, PSFs_RGB, deconved, batch_img_temp, epoch=epoch_id)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_cap_dir', type=str, default='cap_redcolorbar_p10_v1', help='save perturbed images')
    parser.add_argument('--save_dec_dir', type=str, default='dec_redcolorbar_p10_v1', help='save deblurred images')
    parser.add_argument('--save_temp_dir', type=str, default='process_temp', help='save processing temp images')
