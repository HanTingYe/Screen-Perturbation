...
def load_data():
    ...
    train_ds = tf.data.Dataset.list_files('ImageNet/*.JPEG', shuffle=False)
...
def optimize_pattern_with_data(opt):
    ...
    display_interference_pre = crop_image(cv2.resize(np.array(u1_RGB_real_mask), None, fx=10, fy=10, interpolation=cv2.INTER_AREA), img_height, img_width) #INTER_LINEAR INTER_AREA
    ...
    all_interfer_pixel_index = np.unique(u1_RGB_real_mask)
    ...
    Red_mask = np.zeros_like(u1_RGB_real_mask)
    Green_mask = np.zeros_like(u1_RGB_real_mask)
    Blue_mask = np.zeros_like(u1_RGB_real_mask)

    for iii in range(len(red_pixel_index[0])):
        Red_mask = np.where(u1_RGB_real_mask == interfer_pixel_index[red_pixel_index[0][iii]], interfer_pixel_index[red_pixel_index[0][iii]], Red_mask)

    for jjj in range(len(green_pixel_index[0])):
        Green_mask = np.where(u1_RGB_real_mask == interfer_pixel_index[green_pixel_index[0][jjj]], interfer_pixel_index[green_pixel_index[0][jjj]], Green_mask)

    for kkk in range(len(blue_pixel_index[0])):
        Blue_mask = np.where(u1_RGB_real_mask == interfer_pixel_index[blue_pixel_index[0][kkk]], interfer_pixel_index[blue_pixel_index[0][kkk]], Blue_mask)



    interfer_pixel = np.ones(np.shape(interfer_pixel_index), dtype=np.float32)
    interfer_pixel = tf.constant(interfer_pixel)
